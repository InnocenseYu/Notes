...
- static 修饰全局变量，使得这个**全局变量**只能在自己所在的源文件.c内部可以使用，同工程其他源文件**不能**使用
...
int a = 1;
char* p = (char*)&a; // 将 &a 指向 int 类型强制转化为指向 char 类型，虽然 a 是int 类型，4byte, 但是拿值时是 从低字节开始，1byte 1byte 的拿，如果低位1byte 的值等于 1, 则为小端模式，否则位大端模式
