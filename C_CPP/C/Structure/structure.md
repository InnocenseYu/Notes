...
struct Node
{
  ...
  int data;
  //struct Node next; // 错误定义，类似递归，持续的扩充data变量，造成栈溢出
...
{
  int data;
  Node* next; // 错误定义，无标记名 的匿名结构体 typedef 别名，当前使用的别名 Node 仍未定义就拿来使用
}Node;
...
- 结构体指针变量 sp-> 成员 访问结构体内部变量
  ...
  - 只能在 紧贴指针变量名 tmp 后面才能加 ->, -> 访问到的结构体成员名 s 后面不能使用 ->, 需要使用 . 才能操作
...
// 只有在 指针变量名 tmp 后面才能加 ->, 不能在成员名 s 后面加 ->, 需要使用 . 操作
