...
char* p5 = &num;
p5++; //0x04
p5 = (short*) p5; //将 p5 char类型指针强制转换为指向类型为short两个字节且自身类型为short*的类型
printf("*p5 = %#x\n", *p5)

定义p5指针变量char 1个字节类型，char* p5, 每次+1可以取一个，p5+1取值为0x03, 使用*(short*)p5 将p5强制转换为指向类型为short 两个字节，自身类型为short*的类型，进而读出0x03 0x02
```
